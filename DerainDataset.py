...
def prepare_data_real_test(data_path, patch_size, stride):
    # train
    print('process training data')
    input_path = os.path.join(data_path)
    target_path = os.path.join(data_path)
    ...
    for i in range(1000):
        ...
        if os.path.exists(os.path.join(target_path+'/gt', target_file)):

            target = cv2.imread(os.path.join(target_path+'/gt', target_file))
            b, g, r = cv2.split(target)
            target = cv2.merge([r, g, b])

            input_file = "%03d.png" % (i)

            if os.path.exists(os.path.join(input_path+'/rain', input_file)):  # we delete 546 samples

                input_img = cv2.imread(os.path.join(input_path+'/rain', input_file))
                b, g, r = cv2.split(input_img)
                input_img = cv2.merge([r, g, b])
